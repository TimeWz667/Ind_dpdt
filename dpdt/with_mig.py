...
def ode_func(t, y, pars):
    ...
    mr = pars['r_mig'] * np.exp((t - 2015) * pars['rt_mig'])
    return (br + mr - dr) * y
...
def _fn(x, dbr, pop, ode):
    pars = dict(r_die=dbr['DR'], rt_die=dbr['dDR'],
                r_bir=dbr['BR'], rt_bir=dbr['dBR'],
                r_mig=x[0], rt_mig=x[1], year0=2015)
...
def find_pars(dbr, pop):
    opt = minimize(_fn, np.array([0, 0]), args=(dbr, pop, ode_func, ),
                   method='L-BFGS-B', bounds=[(-0.1, 0.1), (-0.1, 0.1)])
...
def reform_pars_to_interp(p0):
    ...
    r_mig, rt_mig = p0['r_mig'], p0['rt_mig']
    p1['RateMig'] = np.array([r_mig * np.exp((t - t0) * rt_mig) for t in years])
...
def regenerate_interp(pars):
    ...
    pars_interp = {
        ...
        'RateMig': interp1d(x=pars['Year'], y=pars['RateMig'])
    }
