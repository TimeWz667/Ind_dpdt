...
from dpdt.without_mig import find_pars, reform_pars_to_interp, regenerate_interp
import os
...
os.makedirs('../pars/without_mig/', exist_ok=True)
os.makedirs('../figs/without_mig/', exist_ok=True)
...
for loc in locations:
    ...
    plt.savefig(f'../figs/without_mig/g_fitness_{lab_loc}.png')
    plt.show()


    with open(f'../pars/without_mig/pars_{lab_loc}.json', 'w') as f:
        pars_json = {k: list(v) for k, v in pars.items()}
        json.dump(pars_json, f)

    with open(f'../pars/without_mig/pars_{lab_loc}.pkl', 'wb') as f:
